...
src/
  app/
    ...
    actions.js               # Server actions: setEarnedMedia, saveNote
    dashboard/
      page.js                # Server component — fetches articles + districts, resolves user district
      DashboardClient.js     # Client component — all dashboard UI, charts, table, modals
    login/
      page.js                # Supabase auth sign-in form
  lib/
    data.js                  # Server-side data fetching via admin client (getArticles, getDistricts)
    supabase/
...
- 3 charts: mention trend (area), source breakdown (donut), sentiment trend (line) — computed from real data
...
- Sidebar district switcher (admin only) — filters everything including charts
