...
import { getDistricts, getSocialAffiliateAccounts, getSocialAffiliateClaims, getSocialAffiliatePreviews } from '@/lib/data';
...
export default async function AffiliateAccountsPage({searchParams}){
 ...
 const [accounts,claims]=districtId?await Promise.all([getSocialAffiliateAccounts(districtId),getSocialAffiliateClaims(districtId)]):[[],[]];
 const previews=districtId?await getSocialAffiliatePreviews(districtId,claims):[];
 return <AffiliateAccountsClient districtId={districtId} districtName={district?.name||'Selected district'} districts={districts} accounts={accounts} claims={claims} previews={previews}/>;
}
