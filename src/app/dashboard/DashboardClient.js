...
function nativeSocialMetric(result, key) {
  const metric = result?.providerMetadata?.native_metrics?.[key];
  return metric && typeof metric === 'object' ? metric : null;
}

function nativeSocialMetricValue(result, key) {
  const metric = nativeSocialMetric(result, key);
  return metric?.availability === 'available' && metric.value !== null && metric.value !== undefined && Number.isFinite(Number(metric.value)) ? Number(metric.value) : null;
}

function nativeSocialScopeLabel(metric) {
  if (!metric) return 'N/A';
  if (metric.sourceScope === 'total') return 'Total';
  if (metric.sourceScope === 'organic') return 'Organic';
  if (metric.sourceScope === 'paid') return 'Paid';
  return 'Attribution not separated';
}

function nativeSocialObservedLabel(metric) {
  const date = new Date(metric?.observedAt || '');
...
function renderedSocialMediaUrl(value, sourceValue = '') {
  const safeUrl = safeSocialMediaUrl(value);
  if (!safeUrl) return '';
  const sourceUrl = safeSocialUrl(sourceValue);
  return sourceUrl ? `/api/social-media?url=${encodeURIComponent(safeUrl)}&source=${encodeURIComponent(sourceUrl)}` : safeUrl;
}
...
function socialCsvRow(result) {
  ...
  return [
    ...
    metricValue('shares'),
    metricValue('views'),
    nativeSocialMetricValue(result, 'uniqueViewers') ?? 'N/A',
    nativeSocialMetricValue(result, 'clicks') ?? 'N/A',
    ...
    nativeSocialMetric(result, 'comments')?.observedAt || 'N/A',
    nativeSocialMetric(result, 'shares')?.observedAt || 'N/A',
    nativeSocialMetric(result, 'clicks')?.observedAt || 'N/A',
    nativeSocialMetric(result, 'saves')?.observedAt || 'N/A',
    nativeSocialMetric(result, 'reposts')?.observedAt || 'N/A',
    nativeSocialMetric(result, 'views')?.availability || 'N/A',
    nativeSocialMetric(result, 'uniqueViewers')?.availability || 'N/A',
    nativeSocialMetric(result, 'reactions')?.availability || 'N/A',
    nativeSocialMetric(result, 'comments')?.availability || 'N/A',
    nativeSocialMetric(result, 'shares')?.availability || 'N/A',
    nativeSocialMetric(result, 'clicks')?.availability || 'N/A',
    nativeSocialMetric(result, 'saves')?.availability || 'N/A',
    nativeSocialMetric(result, 'reposts')?.availability || 'N/A',
    nativeSocialScopeLabel(nativeSocialMetric(result, 'views')),
    nativeSocialScopeLabel(nativeSocialMetric(result, 'uniqueViewers')),
    nativeSocialScopeLabel(nativeSocialMetric(result, 'reactions')),
    nativeSocialScopeLabel(nativeSocialMetric(result, 'comments')),
    nativeSocialScopeLabel(nativeSocialMetric(result, 'shares')),
    nativeSocialScopeLabel(nativeSocialMetric(result, 'clicks')),
...
function SettingsView({ userDistrictId, districts, billingInfo = null, publicPricingLabel = 'Annual access', onPayByCard = null, metaIntegrationEnabled = false }) {
        ...
        <div style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-secondary)', borderRadius: 'var(--radius-lg)', padding: '32px', maxWidth: '800px', marginBottom: '24px' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '16px', marginBottom: '8px' }}>
            <div>
              ...
              <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', lineHeight: 1.6, margin: 0 }}>
                Authorize official Facebook Pages and linked Instagram professional accounts for a future read-only reporting release. Native post synchronization is not yet enabled.
              </p>
            </div>
          ...
          <div style={{ marginTop: '20px', padding: '16px', border: '1px solid var(--border-secondary)', borderRadius: 'var(--radius-md)', color: 'var(--text-secondary)', fontSize: '0.86rem', lineHeight: 1.55 }}>
            <strong style={{ color: 'var(--text-primary)' }}>Workspace:</strong> {integrationDistrict?.name || 'Choose a district on the integrations page'}<br />
            Canary cannot publish posts, reply to comments, send messages, manage ads, or spend advertising budget.
          </div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '20px' }}>
            <a className="btn btn-primary" href={integrationDistrictId
              ? `/dashboard/integrations?districtId=${encodeURIComponent(integrationDistrictId)}`
              : '/dashboard/integrations'}>Manage Meta connection</a>
          </div>
        </div>
      )}
...
function AdminBillingView({ overview = { rows: [], summary: null } }) {
  ...
  const paymentMethodLabel = (value) => value === 'stripe_card'
    ? 'Stripe/card'
    : value === 'check'
      ? 'Check'
      : value === 'complimentary'
  ...
  function saveAttribution(event, billingRow) {
    ...
    startSaving(async () => {
      try {
        ...
        setSaveMessage(`Saved CRM ownership and attribution for ${billingRow.organizationName}.`);
      } catch (error) {
        setSaveMessage(error?.message || 'Could not save CRM ownership and attribution.');
      }
    });
  }

  function exportFilteredCsv() {
    const csv = buildAdminBillingCsv(filteredRows);
    const href = URL.createObjectURL(new Blob([csv], { type: 'text/csv;charset=utf-8' }));
    const anchor = document.createElement('a');
    anchor.href = href;
    anchor.download = `canary-admin-billing-${new Date().toISOString().slice(0, 10)}.csv`;
  ...
  return (
    <div className="data-section">
        ...
        <div>
          ...
          <details style={{ color: 'var(--text-secondary)', fontSize: '0.8rem', marginTop: '8px', maxWidth: '900px' }}>
            ...
            <p style={{ lineHeight: 1.6 }}>
              PO status checks whether a usable-looking PO is stored. Payment and Trial are billing lifecycle states. Paid means a confirmed Stripe payment or a verified deposited check; a stored paid flag without that evidence is shown as verification needed. Payment method is inferred only from protected lifecycle evidence and otherwise remains Unknown. Access controls product availability and does not automatically follow payment. Follow-up explains records requiring human action and its owner records who is responsible. Trial dates show the trial window. Paid date and Paid through show confirmed payment coverage. Sales attribution records the owner and whether the organization is commission-eligible.
            </p>
          </details>
        </div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', alignItems: 'center', marginBottom: '16px' }}>
        ...
        <button type="button" className="btn btn-sm btn-secondary" onClick={exportFilteredCsv}>
          Export filtered CSV
        </button>
      ...
      <div style={{ overflowX: 'auto' }}>
        <table className="data-table" style={{ minWidth: '1650px' }}>
          <thead><tr>
            <th>Organization</th><th>PO status</th><th>Payment</th><th>Payment method</th><th>Trial</th><th>Access</th><th>Follow-up</th>
            <th>Trial dates</th><th>Paid date</th><th>Paid through</th><th>Follow-up owner / sales attribution</th>
          </tr></thead>
          <tbody>
            {filteredRows.map((billingRow) => {
              ...
              return (
                <tr key={billingRow.id}>
                  ...
                  <td>{paymentMethodLabel(billingRow.paymentMethod)}</td>
                  ...
                  <td>
                    {editable ? (
                      <form onSubmit={(event) => saveAttribution(event, billingRow)} style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                        ...
                        <input
                          aria-label={`Follow-up owner for ${billingRow.organizationName}`}
                          name="follow_up_owner"
                          defaultValue={billingRow.followUpOwner}
            ...
            {filteredRows.length === 0 && <tr><td colSpan="11" style={{ color: 'var(--text-secondary)', textAlign: 'center' }}>No billing records match these filters.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
...
function SocialPostPreviewCard({ result, source, rank = null, showContext = false, compact = false, listCompact = false, correctionEnabled = false }) {
  ...
  const classificationLabel = result.relationshipType === 'owned' ? 'Official district post' : (result.relationshipLabel || 'Public mention');
      ...
      <article className={`social-post-preview-card${compact ? ' compact' : ''}`}>
        <header className="social-post-preview-header">
          ...
          <div className="social-post-preview-badges">
            ...
            <span className="social-content-badge">{classificationLabel}</span>
            ...
            {result.visibilityStatus === 'excluded' && <span className="social-review-badge excluded">Hidden as irrelevant</span>}
          </div>
        </header>
...
function SocialReportCard({ result, rank }) {
  ...
  const renderedThumbnailUrl = renderedSocialMediaUrl(thumbnailUrl, sourceUrl);
...
function BoardReportView({ districtName, mediaArticles, socialResults, isEarned }) {
  ...
  return (
    <section className="board-report" aria-label={`${districtName} Board Report`}>
      <header className="board-report-header">
        <div><span>Canary Data</span><h1>{districtName} Board Report</h1><p>Media Intelligence, Strategic Alignment, and approved Social evidence · {new Date().toLocaleDateString()}</p></div>
      </header>
      <div className="board-report-kpis">
        <article><span>News mentions</span><strong>{newsArticles.length}</strong></article>
        <article><span>Average Canary score</span><strong>{averageScore}</strong></article>
        <article><span>Strategic hits</span><strong>{strategicHits.length}</strong></article>
        <article><span>Earned media</span><strong>{earnedCount}</strong></article>
      </div>
...
function MonthlySocialPerformance({
  ...
  return (
    <section className="social-monthly-performance" id="social-monthly-performance" aria-label="Monthly Social Performance">
      ...
      <details className="social-monthly-analyst-note social-monthly-analyst-note-top">
        <summary><span><strong>Add report insight</strong><small>Optional leadership context for the monthly PDF</small></span><em>{analystNote ? 'Draft saved' : 'Optional'}</em></summary>
        <div><span className="social-eyebrow">Analyst insight</span><h3>Social Media Brief</h3><p>This human-written note appears near the top of the monthly PDF.</p></div>
          ...
          <table>
            ...
            <tbody>
              {sortedPosts.length ? sortedPosts.map((post) => {
                ...
                const reactions = socialReportMetricValue(post, 'reactions');
                const comments = socialReportMetricValue(post, 'comments');
                const shares = socialReportMetricValue(post, 'shares');
...
export function SocialView({ socialResults, legacySocialResults = [], socialSources, socialAccountMetricSummaries = {}, socialPerformanceHistory = {}, socialReviewEvents = [], districtFilter, districts, campaignSearch, setCampaignSearch, isAdmin = false, reportAsOf = null, demoMode = false }) {
  ...
  const accountMetricSummary = districtFilter === 'All'
    ? { platforms: {}, platformCount: 0, combinedReachOrViewers: null }
    : socialAccountMetricSummaries[districtFilter] || { platforms: {}, platformCount: 0, combinedReachOrViewers: null };
  ...
  const visibleResults = useMemo(() => {
    ...
    return [...facetedResults].sort((a, b) => {
  ...
  }, [facetedResults, socialSort, sourceByDistrictPlatform]);
  ...
  function exportOfficialSocialCsv() {
    downloadCsv(
  ...
  function exportPublicConversationCsv() {
    downloadCsv(
  ...
  function exportSocialPdf() {
    if (districtFilter === 'All') {
      setSocialMessage('Choose one district before exporting a Social Report.');
      return;
    }
  ...
  return (
    <div className={`social-monitor-view${socialReportMode ? ' social-report-mode' : ''}`}>
      ...
      <nav className="social-page-tabs" aria-label="Social page sections">
        <button type="button" className={socialPageTab === 'overview' ? 'active' : ''} aria-pressed={socialPageTab === 'overview'} onClick={() => setSocialPageTab('overview')}>
          <strong>Our Social</strong><span>What we are saying about ourselves</span>
        </button>
        <button type="button" className={socialPageTab === 'feed' ? 'active' : ''} aria-pressed={socialPageTab === 'feed'} onClick={() => setSocialPageTab('feed')}>
          <strong>Public conversation</strong><span>What the world is saying about us</span>
        </button>
      </nav>
      {socialPageTab === 'overview' && <>
      ...
      <section className="social-monitor-hero">
        <div>
          <span className="social-eyebrow">Public listening</span>
          <h2>What the world is saying about us</h2>
          <p>Review public posts that tag, mention, or discuss the district. Official district publishing remains available under Our Social.</p>
        </div>
      ...
      <section className="social-results-section" id="social-results">
        <div className="social-section-heading">
          <div>
            <h3>Public conversation table</h3>
            <p>Public tags, mentions, and broader conversation remain clearly classified. Metrics show N/A when the source did not provide that field.</p>
          </div>
...
export default function DashboardClient({ articles, legacySocialAuditArticles = [], districts, queries: initialQueries, clients = [], adminBillingOverview = { rows: [], summary: null }, userDistrictId, initialDistrictId = null, initialView = 'dashboard', paymentNotice = null, billingInfo = null, publicPricingLabel = 'Annual access', publicPricingIntroductory = false, excludedStories = [], correctionEvents = [], socialSources = [], socialThreads = [], socialAccountMetricSummaries = {}, socialPerformanceHistory = {}, socialReviewEvents = [], strategicProfiles = [], strategicPriorities = [], collectionHealth = [], socialCollectionHealth = [], dataWarnings = [], isAdmin = false, isDemoReviewer = false, reviewerDistrictCount = 0, melodiEnabled = false, metaIntegrationEnabled = false, demoMode = false, socialReportAsOf = null }) {
  ...
  function handleBoardReportPdf() {
    ...
    setFeedbackOpen(false);
    setSidebarOpen(false);
    setBoardReportMode(true);
    const restore = () => setBoardReportMode(false);
    window.addEventListener('afterprint', restore, { once: true });
          ...
          <div className="topbar-right">
            ...
            {SHOW_GLOBAL_BOARD_REPORT_EXPORT && ['dashboard', 'birdseye', 'social'].includes(currentView) && (
              <button
                className="btn btn-secondary btn-sm export-pdf-btn"
                type="button"
                onClick={handleBoardReportPdf}
                disabled={districtFilter === 'All'}
                title={districtFilter === 'All' ? 'Choose one district to create a Board Report.' : 'Combines Media Intelligence, Strategic Alignment, approved Social, and an evidence appendix.'}
              >
                ⬇ Board Report PDF
              </button>
            )}
          ...
          <section className="communications-brief" aria-label="Communications brief">
            <header className="communications-brief-header">
              <div>
                ...
                <p>Media values follow the active filters. Social cues include all enriched results for the selected district. Recommendations remain advisory and require source verification.</p>
              </div>
          ...
          <div className="data-section">
            ...
            <div className="filter-secondary-bar">
              ...
              <div className="filter-control-group">
                ...
                {strategicAlignmentFilter !== 'All' && (
                  <button type="button" className="active-filter-chip" onClick={() => setStrategicAlignmentFilter('All')} title="Remove Strategic Alignment filter">
                    {strategicAlignmentFilter} <span aria-hidden="true">×</span>
                  </button>
                )}
              </div>
