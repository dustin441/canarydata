...
function buildContextText({ district, profile, priorities, news, social, appliedWindowDays }) {
  ...
  news.forEach((item) => {
    lines.push(`[${item.citationId}] News | ${dateValue(item.date)} | source ownership=${item.is_earned_media === true ? 'external coverage' : item.is_earned_media === false ? 'district-owned coverage' : 'not reviewed'} | communications earned=${item.communications_earned === true ? 'communicator marked earned' : 'not marked earned'} | ${compact(item.headline, 260)} | source=${compact(item.source, 100)} | summary=${compact(item.summary, 550)} | Canary Score=${item.canary_score ?? 'N/A'} | strategic alignment=${compact(item.innovation_reason, 350)} | review-only recommendation=${compact(item.recommendation, 450)}`);
  });
  social.forEach((item) => {
