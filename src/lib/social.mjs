...
export function normalizeSocialResult(item = {}) {
  ...
  return {
    ...
    reviewVersion: Number.isInteger(item.review_version) ? item.review_version : 0,
    ...
    hasComparableInteractionData: providerMetadata.native_interaction_coverage !== 'partial' && ['reactions', 'comments', 'shares'].every((metric) => metricAvailability[metric] === true),
