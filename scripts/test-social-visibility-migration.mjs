...
try{
 ...
 assert.equal(evidenceSource.correctionRequests.length,1);assert.ok(evidenceSource.correctionRequests[0].requestPayload);assert.ok(evidenceSource.correctionRequests[0].resultRow);
 ...
 node('scripts/capture-social-rollback-evidence.mjs',['--input',join(temp,'evidence-input.csv'),'--visibility-backup',join(temp,'backup.json'),'--qa-fixture-manifest',join(temp,'qa-manifest.json'),'--output',join(temp,'rollback-evidence.json')]);
 const evidence=JSON.parse(await readFile(join(temp,'rollback-evidence.json'),'utf8'));
 assert.equal(evidence.manifest.correctionRequestCount,1);assert.equal(evidence.manifest.replayRowCount,2);assert.equal(evidence.manifest.qaFixtureDeleteCount,1);
 assert.equal((await stat(join(temp,'rollback-evidence.json'))).mode&0o777,0o600);
 const unbacked=psql(down,false);assert.match(unbacked.stderr,/prepare-social-rollback/i);assert.equal(psql('select count(*) from public.social_correction_requests;').stdout.trim(),'1');
 ...
 assert.equal(psql("select count(*) from social_review_events e join social_review_batches b on b.id=e.batch_id join social_threads t on t.id=e.social_thread_id;").stdout.trim(),'1');
 ...
 const restored=parseJson(psql(`set canary.expected_social_state='N-1';set canary.expected_social_rows='8';set canary.expected_social_exclusions='2';${verify}`).stdout);
 ...
 assert.equal(psql(`select count(*) from social_threads where id='${qaId}';`).stdout.trim(),'0');
 ...
 console.log('Social visibility migration PostgreSQL test passed: CSV handoff, sealed contracts, correction evidence, audit fail-closed preservation, exact restore, N-1 replay, and manifest-only QA deletion.');
