-- Dedicated production N-1 contract fixture captured before Task 4/Task 5.
-- Do not replace this with supabase/social_review_workflow.sql: that file later diverged.
-- Captured pg_get_functiondef() MD5 values:
...
-- This function/trigger are part of the captured production contract but are
-- installed by social_monitoring.sql rather than social_review_workflow.sql.
begin;
...
drop trigger if exists social_threads_touch_updated_at on public.social_threads;
create trigger social_threads_touch_updated_at
