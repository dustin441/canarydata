...
assert.match(actions.slice(deleteQueryStart, deleteQueryStart + 1400), /update\(\{ active: false \}\)/, 'removing a query must archive it instead of deleting history');
assert.match(actions.slice(deleteQueryStart, deleteQueryStart + 1400), /query\.channels !== 'news'/, 'customers must not be able to remove admin-managed advanced queries');
