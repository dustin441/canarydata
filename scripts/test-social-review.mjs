...
assert.match(dashboard, /row\.accountName \|\| row\.accountHandle \|\| 'Authorized account'/,'native account rows must identify the authorized account');
assert.match(dashboard, /<th>Reposts<\/th>/,'native repost metrics must render in the on-screen post table');
assert.match(dashboard, /repostMetric \? <NativeAccountMetricCell metric=\{repostMetric\}/,'native repost cells must preserve availability and source-window labels');
